...
function processImage() {
	...
	findNotOrg1(); // オルガノイド本体とは異なる領域を明らかにする
	closeCyst(); // Org1.tifを入力として、もう少しで閉じそうな辺縁部のCystを閉じて、Result of Outlineを作成する
	findNotOrg2(); // オルガノイド本体に接している領域で、オルガノイド本体とは異なる領域を明らかにする
	analyseOrg(); // Org2.tifを入力として面積の計測までを行い、"Results"を表示する
	largeCentroid = getCentroid(0.83, true); // "Results"から、重心を特定するのに必要な値をピクセル数として取得する。Centroid = [Minor, alpha, X, Y]
...
function closeCyst() {
	selectWindow("notOrg1.tif"); // 最初にnotOrgの領域を削除する
	run("Create Selection");
	selectWindow("Org1.tif");
	run("Restore Selection");
	run("Clear", "slice");
...
function findNotOrg2() {
	selectWindow("notOrg2.tif");
	run("Create Selection");
	selectWindow("Result of Outline.tif");
	run("Duplicate...", "title=notOrg3.tif");
	run("Restore Selection");
	run("Clear", "slice");
	run("Select None");
	run("Gaussian Blur...", "sigma=8"); // ぼかしが強いとOrgとnotOrgが融合してExludeされてしまう
	...
	imageCalculator("OR create", "notOrg1.tif", "notOrg3.tif");
...
function findPeripheral2(sigma) {
	...
	run("Create Selection");
	selectWindow("Otsu.tif");
	for (j = 0; j < 7; j++) { // もともとnoisyなCyst以外の辺縁部の明るい領域は、次のOpen処理で大きく削られることになる（7は必要そう）
		run("Salt and Pepper");
	}
	run("Restore Selection");
	run("Clear Outside");
...
function findCenter(sigma) {
	...
	run("Analyze Particles...", "size=0.3-Infinity circularity=0.3-1.00 show=Masks");
