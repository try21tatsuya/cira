...
for (i = 0; i < files.length; i++) {
	...
	if (!File.isDirectory(fname)) {
		...
		run("Median...", "radius=1"); //離れた位置にある細かい点はノイズと考えて除去する
		run("Fill Holes");
		...
		// Orgの画像とCystの画像を組み合わせて、オルガノイド全体の領域を特定する
		imageCalculator("OR create", "Org.tif", "Cyst.tif");
		...
		selectWindow("Cyst.tif");
		run("Create Selection");
		selectWindow("Org.tif");
		run("Restore Selection");
		run("Clear");
		saveAs(".tif", fname + "_Result2.tif");
