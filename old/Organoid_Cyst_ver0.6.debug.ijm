...
function findOrg(radius) {
	...
	makeRectangle(0, 0, 1920, 1); // オルガノイドが画像の上下に接してしまっている場合にexcludeされないようにする処理
	run("Clear", "slice");
...
function findPeripheral(radius) {
	...
	run("Analyze Particles...", "size=0.05-30 circularity=0.4-1.00 show=Masks"); // ある程度大きいCystのみに限定、circularityが意外と大事
...
function findCenter(radius) {
	...
	run("Analyze Particles...", "size=0.2-30 circularity=0.3-1.00 show=Masks"); // ある程度大きいCystのみに限定、circularityが意外と大事
