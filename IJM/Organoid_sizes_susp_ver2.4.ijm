...
for (i = 0; i < files.length; i++) {
	...
	if (!File.isDirectory(f)) {
		...
		run("Convert to Mask");
		run("Set Measurements...", "area mean fit shape integrated limit redirect=None decimal=2"); // 平均輝度などを追加
		...
		run("Erode");
		run("Erode");
		run("Analyze Particles...", "size=0.4-48 show=Masks exclude clear"); //clearで1回目の結果（1回目は'display'していないが、裏で記録されている）を消去、今回はexcludeする
